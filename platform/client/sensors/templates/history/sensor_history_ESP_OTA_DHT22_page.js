Template.sensor_history_ESP_OTA_DHT22_page.events({});

Template.sensor_history_ESP_OTA_DHT22_page.helpers({
    'humidity': function () {
        var cols = [];
        var col = ["humidity"];
        var chipid = Router.current().params._chipid;
        var data = _.pluck(_.pluck(_.pluck(History.find({'message.ChipId': chipid}).fetch(), "message"), "d"), "Humidity");
    ...
    'ESP_OTA_DHT22_table': function () {
        return {
            ...
            fields: [
                ...
                {key: 'message.d.Humidity', label: 'Humidity'},
                {key: 'message.d.Temperature', label: 'Temperature'},
...
function drawChart(myId) {
    var data = [
        {
            value: History.find({'message.ChipId': Router.current().params._chipid}).count(),
            color: "darkred",
            highlight: "#2ECC71",
            label: "MY"
        },
...
function drawChart3(myId) {
    ...
    var cur = History.find({'message.ChipId': Router.current().params._chipid}, {
        sort: {date_created: -1},
        skip: 0,
        limit: 20
    });
